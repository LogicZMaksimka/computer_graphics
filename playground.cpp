...
glm::mat4 CreateMVPMatrix(glm::vec3 pos, glm::vec3 scaling_vec, glm::vec3 rotation_dir, float angle)
{
  // Model transformations
  glm::mat4 translate = glm::translate(glm::mat4(), pos);
  glm::mat4 rotate = glm::rotate(glm::radians(angle), rotation_dir);
  glm::mat4 scale = glm::scale(glm::mat4(), scaling_vec);
...
void DrawStationaryTriangles(
    std::pair<GLuint, GLuint> triangle_VBO_VAO,
    GLuint red_shader_program,
    GLuint red_glsl_mvp_matrix,
    GLuint green_shader_program,
    GLuint green_glsl_mvp_matrix)
{
  // First triangle
  glm::mat4 mvp_matrix = CreateMVPMatrix(glm::vec3(-0.2f, 0.0f, 0.0f), glm::vec3(0.5f), glm::vec3(0.0f, 1.0f, 0.0f), 10.0f);
  Draw(triangle_VBO_VAO.second, red_shader_program, red_glsl_mvp_matrix, 3, mvp_matrix);

  // Second triangle
  mvp_matrix = CreateMVPMatrix(glm::vec3(0.2f, 0.0f, 0.0f), glm::vec3(0.5f), glm::vec3(0.0f, 1.0f, 0.0f), -10.0f);
  Draw(triangle_VBO_VAO.second, green_shader_program, green_glsl_mvp_matrix, 3, mvp_matrix);
}

void DrawMovingTriangles(
    std::pair<GLuint, GLuint> triangle_VBO_VAO,
    GLuint red_shader_program,
    GLuint red_glsl_mvp_matrix,
    GLuint green_shader_program,
...
{
  // First triangle
  glm::mat4 mvp_matrix = GetProjectionMatrix(window_width, window_height) * GetViewMatrix() * CreateMVPMatrix(glm::vec3(-0.2f, 0.0f, 0.0f), glm::vec3(0.5f), glm::vec3(0.0f, 1.0f, 0.0f), 10.0f);
  Draw(triangle_VBO_VAO.second, red_shader_program, red_glsl_mvp_matrix, 3, mvp_matrix);

  // Second triangle
  mvp_matrix = GetProjectionMatrix(window_width, window_height) * GetViewMatrix() * CreateMVPMatrix(glm::vec3(0.2f, 0.0f, 0.0f), glm::vec3(0.5f), glm::vec3(0.0f, 1.0f, 0.0f), -10.0f);
  Draw(triangle_VBO_VAO.second, green_shader_program, green_glsl_mvp_matrix, 3, mvp_matrix);
}

void DrawTetrahedron(
    std::pair<GLuint, GLuint> tetrahedron_VBO_VAO,
    GLuint fractal_shader_program,
    GLuint fractal_glsl_time,
    GLuint fractal_glsl_mvp_matrix,
    int window_width,
    int window_height)
{
...
void DrawMovingNonIntersectingTriangles(
    std::pair<GLuint, GLuint> triangle_VBO_VAO,
    GLuint red_shader_program,
    GLuint red_glsl_mvp_matrix,
    GLuint green_shader_program,
...
{
  // First triangle
  glm::mat4 mvp_matrix =  CreateMVPMatrix(glm::vec3(-0.2f, 0.0f, 0.2f), glm::vec3(0.5f), glm::vec3(0.0f, 1.0f, 0.0f), 0.0f);
  Draw(triangle_VBO_VAO.second, red_shader_program, red_glsl_mvp_matrix, 3, mvp_matrix);

  // Second triangle
  mvp_matrix = CreateMVPMatrix(glm::vec3(0.2f, 0.0f, 0.0f), glm::vec3(0.5f), glm::vec3(0.0f, 1.0f, 0.0f), 0.0f);
  Draw(triangle_VBO_VAO.second, green_shader_program, green_glsl_mvp_matrix, 3, mvp_matrix);
}
...
int main()
{
  ...
  while (/*ESC button*/ glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS && /*red cross*/ !glfwWindowShouldClose(window))
  {
    ...
    // DrawStationaryTriangles(triangle_VBO_VAO, red_shader_program, red_glsl_mvp_matrix, green_shader_program, green_glsl_mvp_matrix);
    ...
    // DrawMovingTriangles(triangle_VBO_VAO, red_shader_program, red_glsl_mvp_matrix, green_shader_program, green_glsl_mvp_matrix, window_width, window_height);
    ...
    DrawTetrahedron(tetrahedron_VBO_VAO, fractal_shader_program, fractal_glsl_time, fractal_glsl_mvp_matrix, window_width, window_height);


    // DrawMovingNonIntersectingTriangles(triangle_VBO_VAO, red_shader_program, red_glsl_mvp_matrix, green_shader_program, green_glsl_mvp_matrix, window_width, window_height);
